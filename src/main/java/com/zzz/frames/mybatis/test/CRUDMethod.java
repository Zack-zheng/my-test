...
import com.zzz.frames.mybatis.inter.IUserOperation;
import com.zzz.frames.mybatis.models.Article;
...
public class CRUDMethod {
    ...
    static{
        try{
            reader    = Resources.getResourceAsReader("com/zzz/frames/mybatis/Configuration.xml");
    ...
    public void selectByIdNamespaceWay(int id){
        SqlSession session = sqlSessionFactory.openSession();
        try {
            User user = (User) session.selectOne("com.zzz.frames.mybatis.inter.UserMapper.selectUserByID", id);
            System.out.println(user.getUserAddress());
            System.out.println(user.getUserName());
        } finally {
            session.close();
